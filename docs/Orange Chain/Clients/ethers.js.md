...
You can connect to Base by instantiating a new ethers.js `JsonRpcProvider` object with a RPC URL of the Orange network:

```javascript
const ethers = require('ethers');

const url = 'https://rpc.orangechain.xyz';
...
In order to write data to the Orange network, you need to create a `Signer`.
...
You can use ethers.js to interact with a smart contract on Orange by instantiating a `Contract` object using the ABI and address of a deployed contract:
